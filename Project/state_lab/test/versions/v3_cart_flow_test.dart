...
import 'package:state_lab/versions/v3_getx/v3_shop_root.dart';
...
void main() {
  // GetX 是全局注册表：不清会跨用例串味（这正是讲义里点名的代价之一）
  tearDown(Get.reset);

  testWidgets('v3 主流程：列表加载 → 加购 → 角标 → 购物车增减/合计 → 清空', (tester) async {
    await tester.pumpWidget(MaterialApp(home: V3ShopRoot(api: FakeProductApi())));
    await tester.pump(); // Get.put → onInit → loadFirst 落地
    await tester.pump(); // Future 完成后 update 落地
  ...
  testWidgets('S0 学员 bug 剧本在 v3 上回归：详情 → 清空 → 返回，角标清零', (tester) async {
    await tester.pumpWidget(MaterialApp(home: V3ShopRoot(api: FakeProductApi())));
    await tester.pump();
    await tester.pump();
