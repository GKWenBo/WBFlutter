...
import 'package:state_lab/versions/v4_riverpod/v4_shop_root.dart';
...
void main() {
  testWidgets('v4 主流程：列表加载 → 加购 → 角标 → 购物车增减/合计 → 清空', (tester) async {
    await tester.pumpWidget(MaterialApp(home: V4ShopRoot(api: FakeProductApi())));
    await tester.pump(); // AsyncNotifier.build 开跑
    await tester.pump(); // Future 完成，AsyncData 落地
  ...
  testWidgets('S0 学员 bug 剧本在 v4 上回归：详情 → 清空 → 返回，角标清零', (tester) async {
    await tester.pumpWidget(MaterialApp(home: V4ShopRoot(api: FakeProductApi())));
    await tester.pump();
    await tester.pump();
